...
namespace Games
{
    public class Square : IState
    {
        ...
        public IEnumerable<ISuccessor> GetSuccessors()
        {
            for (int i = 0; i < 4; i++)
            {
                ...
                if (Test(idx, move))
                {
                    ...
                    yield return new Games.SquareMove(Swap(idx, idx + shift), move);
                }
            }
        }
        ...
        public override string ToString()
        {
            ...
            Func<int, string> f = i => i == 0 ? " " : i.ToString();
            for (int i = 0; i < _square.Length; i+=3)
            {
                sb.Append("+---+---+---+\n");
                sb.Append(string.Format("|{0,2} ", f(_square[i])));
                sb.Append(string.Format("|{0,2} ", f(_square[i+1])));
